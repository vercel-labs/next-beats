...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [showsResponse, episodesResponse] = await Promise.all([
      fetch(`${API}/users/${encodeURIComponent(String(userId))}/shows?limit=${limit}`, { headers, next: { revalidate: 300 } }),
      fetch(`${API}/users/${encodeURIComponent(String(userId))}/episodes?limit=${limit}`, { headers, next: { revalidate: 300 } }),
    ]);
    ...
    return NextResponse.json({ episodes: episodes?.response?.items ?? [], shows: shows?.response?.items ?? [] });
  } catch {
