...
// ---cut---
await Promise.all([ocean.compile({ colors: ['bgra8unorm'] }), boat.compile({ colors: ['bgra8unorm'] })]);
...
canvasTarget.onResize(() => {
  scene = recordScene();
}); // needed only if the bundle samples resized resources
