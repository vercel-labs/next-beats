...
The fix is one call. `clock(gpu).advance(dtSeconds)` moves the vgpu clock forward _now_, and claims that frame's tick: the next `frame(gpu)` counts the frame and runs its passes, but does not advance the clock again. One tick per frame, with the manual one winning.
